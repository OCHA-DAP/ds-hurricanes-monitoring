...
df_obsv_specific = df_obsv[df_obsv["id"] == RAFAEL_ATCF_ID]
```

```python
cols = ["latitude", "longitude", "intensity"]
df_interp = (
    df_obsv_specific.set_index("lastUpdate")[cols]
    .resample("30min")
    .interpolate()
    .reset_index()
)
...
```python
for pcode, row in adm_cub.to_crs(3857).set_index("ADM_PCODE").iterrows():
...
```python
da_sum.rio.clip(adm_cub[adm_cub["ADM1_PCODE"] == HAVANA1].geometry).plot()
```

```python
da_sum.rio.clip(adm_cub[adm_cub["ADM1_PCODE"] == "CU11"].geometry).plot()
```
